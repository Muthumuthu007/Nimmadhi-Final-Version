...
const WeeklyReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      const response = await makeApiRequest<WeeklyReportData>({
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm text-gray-500">
                    ({format(new Date(reportData.report_period.start_date), 'MMM d')} - {format(new Date(reportData.report_period.end_date), 'MMM d, yyyy')})
                  </span>
                </div>
              </div>
