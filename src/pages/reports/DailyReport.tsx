...
import { 
  ...
  Save, Box, Truck, Settings
} from 'lucide-react';
import { makeApiRequest } from '../../utils/api';
import { format, parse } from 'date-fns';
import { useAuth } from '../../contexts/AuthContext';
import { downloadCSV, generateDailyReportCSV } from '../../utils/csvExport';
...
const DailyReport = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      const csvData = generateDailyReportCSV(reportData);
      const filename = `daily-report-${format(new Date(reportData.report_date), 'yyyy-MM-dd')}.csv`;
  ...
  const fetchReport = async () => {
    ...
    try {
      const response = await makeApiRequest<DailyReportData>({
        operation: 'GetDailyReport',
        report_date: selectedDate,
        username: user.username
      });

      if (!response || !response.stock_summary) {
  ...
  const formatTimestamp = (timestamp: string) => {
    try {
      // Parse the timestamp which is in format "YYYY-MM-DD HH:mm:ss AM/PM"
  ...
  const formatTransactionDetails = (details: any, type: string): string => {
    ...
    // Debug the details object for the specific operations
    if (type === 'SaveOpeningStock' || type === 'SaveClosingStock') {
      console.log(`Details for ${type}:`, details);
    }
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm text-gray-500">
                    ({format(new Date(reportData.report_date), 'MMMM d, yyyy')})
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {renderSummaryCard(
                  "Opening Stock",
                  reportData.stock_summary?.opening_stock_qty,
                  reportData.stock_summary?.opening_stock_amount,
                  <Boxes className="h-6 w-6 text-blue-700" />,
                  "bg-blue-50 text-blue-700"
                )}
                {renderSummaryCard(
                  "Consumption",
                  reportData.stock_summary?.consumption_qty,
                  reportData.stock_summary?.consumption_amount,
                  <TrendingDown className="h-6 w-6 text-red-700" />,
                  "bg-red-50 text-red-700"
                )}
                {renderSummaryCard(
                  "Closing Stock",
                  reportData.stock_summary?.closing_stock_qty,
                  reportData.stock_summary?.closing_stock_amount,
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="p-6">
              ...
              <div className="space-y-6">
                {(() => {
                  try {
                    const transactionsByOp = reportData.transactions_by_operation || {};
                    
                    if (Object.keys(transactionsByOp).length === 0) {
                      return (
                    ...
                    return Object.entries(transactionsByOp).map(([operation, transactions]) => (
                      <div key={operation} className="space-y-4">
                        ...
                        <div className="space-y-3">
                          {Array.isArray(transactions) ? transactions.map((transaction: any) => (
                            <div
                              key={transaction.transaction_id || `tx-${Math.random()}`}
                            ...
                            <div className="text-sm text-gray-500">Invalid transaction data</div>
                          )}
                        </div>
                      </div>
                    ));
